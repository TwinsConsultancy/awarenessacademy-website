        ...
        container.innerHTML = coursesWithModules.map(c => {
            ...
            const moduleButtons = c.modules && c.modules.length > 0 
                ? c.modules.slice(0, 3).map(module => `
                    <button onclick="checkAndOpenFeedbackModal('${module._id}', '${(module.title || 'Module').replace(/'/g, '\\\'')}')" 
                            class="btn-secondary" 
...
async function checkAndOpenFeedbackModal(moduleId, moduleName) {
    try {
        ...
        if (typeof openFeedbackModal === 'function') {
            openFeedbackModal(moduleId, moduleName);
        } else {
